...
import { X, Download, ChevronRight, FileText, Trash2 } from 'lucide-react'
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100">
          ...
          <div className="flex items-center gap-4">
            <p className="text-2xl font-bold text-indigo-700">
              {result.total_score}
              <span className="text-sm text-gray-400 font-normal"> / {result.max_total} marks</span>
            </p>
...
export default function Students() {
  ...
  const exportCSV = () => {
    const lines = ['Student,USN,Subject,Marks,MaxMarks,Questions,Date']
    results.forEach(r => {
      lines.push(`"${r.student_name}","${r.student_usn}","${r.subject}",${r.total_score},${r.max_total},${r.questions_evaluated},"${new Date(r.evaluated_at).toLocaleString()}"`)
    })
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-gray-100 bg-gray-50">
                <th className="text-left px-4 py-3 text-xs font-semibold text-gray-500 uppercase">Student / File</th>
                ...
                <th className="text-center px-4 py-3 text-xs font-semibold text-gray-500 uppercase">Questions</th>
              ...
              {filtered.map(r => (
                ...
                >
                  ...
                  <td className="px-4 py-3 text-right">
                    <span className="font-bold text-gray-800 text-base">{r.total_score}</span>
                    <span className="text-xs text-gray-400 font-normal"> / {r.max_total}</span>
                  </td>
